...
def get_projection(data, calculate_surplus_capital=True):
    ...
    def liquidate_in_order(transactions, start_book, person, age, tax_rate, income_limit, amount, order):
        ...
        for account in order:
            if account == Account.REGULAR:
                ...
                meet_cash_req_from_deferred(transactions, book, person, Account.RRIF, parameters["tax_rate"], amount,
                                            income_limit)
            elif account == Account.RRSP:
                meet_cash_req_from_deferred(transactions, book, person, Account.RRSP, parameters["tax_rate"], amount,
                                            income_limit)
                ...
                if age >= 50 and book[person][Account.LIRA] > 0:
                    ...
                    book = process_transactions(start_book, transactions)
                    meet_cash_req_from_lif(transactions, book, person, age, parameters["tax_rate"], amount,
                                           income_limit)
    ...
    def sell_assets_to_meet_needs_strategy_1_w_spouse(transactions, start_book, year):
        ...
        liquidate_in_order(transactions, start_book, "spouse", spouse_age, parameters["tax_rate"], 0, target,
                           [Account.REGULAR, Account.TFSA, Account.RRIF, Account.RRSP, Account.LIF, Account.LIRA])
    ...
    def set_essential_capital(book, essential_capital):
        ...
        for person in persons:
            if book[person][Account.REGULAR] > delta:
                createTransaction(transactions, "debit", person, Account.REGULAR, delta,
                                  TransactionType.REMOVE_SURPLUS_CAPITAL)
                createTransaction(transactions, "debit", person, Account.REGULAR_BOOK_VALUE,
                                  delta * book[person][Account.REGULAR_BOOK_VALUE] / book[person][Account.REGULAR],
                                  TransactionType.REMOVE_SURPLUS_CAPITAL)
                delta = 0

            elif book[person][Account.REGULAR] > 0:
                createTransaction(transactions, "debit", person, Account.REGULAR, book[person][Account.REGULAR],
                                  TransactionType.REMOVE_SURPLUS_CAPITAL)
                createTransaction(transactions, "debit", person, Account.REGULAR_BOOK_VALUE,
                                  book[person][Account.REGULAR_BOOK_VALUE], TransactionType.REMOVE_SURPLUS_CAPITAL)
        ...
        for person in persons:
            ...
            if book[person][Account.TFSA] > delta:
                createTransaction(transactions, "debit", person, Account.TFSA, delta,
                                  TransactionType.REMOVE_SURPLUS_CAPITAL)
                delta = 0
        ...
        for person in persons:
            ...
            if book[person][Account.RRSP] > delta:
                createTransaction(transactions, "debit", person, Account.RRSP, delta,
                                  TransactionType.REMOVE_SURPLUS_CAPITAL)
                delta = 0
        ...
        for person in persons:
            ...
            if book[person][Account.RRIF] > delta:
                createTransaction(transactions, "debit", person, Account.RRIF, delta,
                                  TransactionType.REMOVE_SURPLUS_CAPITAL)
                delta = 0
        ...
        for person in persons:
            ...
            if book[person][Account.LIF] > delta:
                createTransaction(transactions, "debit", person, Account.LIF, delta,
                                  TransactionType.REMOVE_SURPLUS_CAPITAL)
                delta = 0
    ...
    def find_essential_capital(start_book):
        ...
        desired_end_balance = get_future_value(parameters["start_year"], parameters["end_year"],
                                               parameters["end_balance"], parameters["inflation"])
    ...
    surplus = [
        get_capital(surplus_capital_projection[i]["start"]) - get_capital(essential_capital_projection[i]["start"]) for
        i in range(l)]
    surplus.append(
...
def format(d):
    d["parameters"]["tax_rate"] = {"marginal": [(12070, 0.0), (50000, 0.25), (90000, 0.35), (200000, 0.45)],
                                   "top": 0.54}

    d["start_book"] = {
        "joint": {Account.CLEARING: d["start_book"]["joint"]["clearing"],
                  Account.HOME: d["start_book"]["joint"]["home"]},
        "client": {
            Account.REGULAR: d["start_book"]["client"]["regular"],
            Account.REGULAR_BOOK_VALUE: d["start_book"]["client"]["regular_book_value"],
            Account.TFSA: d["start_book"]["client"]["tfsa"],
            Account.RRSP: d["start_book"]["client"]["rrsp"],
            Account.RRIF: d["start_book"]["client"]["rrif"],
            Account.LIF: d["start_book"]["client"]["lif"],
            Account.LIRA: d["start_book"]["client"]["lira"]
        },

        "spouse": {
            Account.REGULAR: d["start_book"]["spouse"]["regular"],
            Account.REGULAR_BOOK_VALUE: d["start_book"]["spouse"]["regular_book_value"],
            Account.TFSA: d["start_book"]["spouse"]["tfsa"],
            Account.RRSP: d["start_book"]["spouse"]["rrsp"],
            Account.RRIF: d["start_book"]["spouse"]["rrif"],
            Account.LIF: d["start_book"]["spouse"]["lif"],
            Account.LIRA: d["start_book"]["spouse"]["lira"]
        }
    }
